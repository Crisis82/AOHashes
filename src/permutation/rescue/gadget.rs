...
use crate::rescue::{ALPHA_INV, MDS_MATRIX, ROUND_CONSTANTS, WIDTH};
...
impl<'a> RescueHash<Witness> for GadgetPermutation<'a> {
    fn sbox_layer(&mut self, state: &mut [Witness]) {
        ...
        for i in 0..WIDTH {
            ...
            // value^5
            let constraint = Constraint::new().mult(1).a(power4).b(state[i]);
...
mod tests {
    ...
    fn rescue() -> ([BlsScalar; WIDTH], [BlsScalar; WIDTH]) {
    ...
    fn setup() -> Result<(Prover, Verifier), Error> {
        ...
        let label = b"rescue_gadget_tester";

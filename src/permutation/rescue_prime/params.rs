use crate::permutation::{BlsScalar, Shake256PRNG, u256};
use crate::rescue_prime::{ROUNDS, WIDTH};
use alloc::format;

/// Rescue-Prime round constants generation function using SHAKE256 with a fixed
/// seed.
pub(crate) fn gen_const() -> [[BlsScalar; 2 * WIDTH]; ROUNDS] {
    let mut constants = [[BlsScalar::zero(); 2 * WIDTH]; ROUNDS];

    // modulus p
    let p = u256::from_str_hex(
    ...
    // fixed seed from the parameters
    let seed =
        format!("Rescue-XLIX({:?},{:?},{:?},{:?})", p, WIDTH, capacity, s)
            .as_bytes()
            .to_vec();

    // PRNG based on SHAKE256
    let mut prng = Shake256PRNG::new(seed.as_slice());
...
/// Rescue-Prime MDS matrix generation function.
pub(crate) fn gen_mds() -> [[BlsScalar; WIDTH]; WIDTH] {
    ...
    let mds = transpose(right_half(matrix));
    mds
}
